...
public class CallLegPromptAndRecordCompletedEventTest {
	@Test
	public void testCallLegPromptAndRecordCompletedEvent() throws Exception {
        CallLegPromptAndRecordCompletedEvent e = new CallLegPromptAndRecordCompletedEvent(new DialogPromptAndRecordCompletedEvent("test", "cmd", "record.complete", "file://fred", "123s"));

		assertEquals("test", e.getId());
		assertEquals("cmd", e.getMediaCommandId());
		assertEquals("record.complete", e.getRecordEnd());
		assertEquals("file://fred", e.getRecordId());
