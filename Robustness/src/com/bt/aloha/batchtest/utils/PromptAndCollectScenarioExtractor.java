...
/**
 * Ad-hoc class to extract some details from app.log for the PromptAndCollectScenario
 * @author 802083751
 */
public class PromptAndCollectScenarioExtractor extends ScenarioExtractor {
    ...
    public PromptAndCollectScenarioExtractor(String filename) {
        super(filename);
        init();
    }
    ...
    public void doit(String scenarioId) throws Exception {
        ...
        if (null != this.callId) {
            for (List<Line> l: this.sipMessages) {
                inner:
                for(Line line: l) {
                    if (line.contains(this.d1) || line.contains(this.d2)) {
                        log(l);
                        break inner;
                    }
                }
    ...
    protected void processLine() throws Exception {
        ...
        if (pass == 1) {
            if (line.contains("Sending request") || line.contains("Sending response") || line.contains("Received response") || line.contains("Received request")){
                processSipMessage();
            }
        }
    }
...
//        new ScenarioExtractor(args[0]).doit(args[1]);
          new PromptAndCollectScenarioExtractor("app.log").doit(
"promptAndCollectScenario:d8d43824334bcfefc510bca0c4245a97");
    }
}
